...
class UserProfile(Common):
    ...
    diploms = models.TextField('Дипломы', default = '')
    class Meta:
        verbose_name = 'Профиль пользователя'
        verbose_name_plural = 'Профили пользователей'

    def show_email(self):
        return format_html('<a href="/admin/auth/user/' + str(self.user.id) + '">' + self.user.email + '</a>')
    show_email.short_description = 'Основной e-mail'
    show_email.admin_order_field = 'user.email'
    
    def send_email(self):
        return format_html('<a href="mailto:' + str(self.user.email) + '">mail to</a><br><a href="/lk/formsuserprofile/delete_email/' + str(self.user.id) + '/">delete email</a>')
...
class Form(Common):      
    ...
    aif = models.ManyToManyField('self', verbose_name='avaliable if (OR)', blank=True, symmetrical=False, related_name='aif_set')
    unaif = models.ManyToManyField('self', verbose_name='unavaliable if (OR)', blank=True, symmetrical=False, related_name='unaif_set')
    aifpass = models.ManyToManyField('self', verbose_name='avaliable if is_passed (OR)', blank=True, symmetrical=False, related_name='aifpass_set')
    aifres = models.ManyToManyField('self', verbose_name='avaliable if is_reserved (OR)', blank=True, symmetrical=False, related_name='aifres_set')
