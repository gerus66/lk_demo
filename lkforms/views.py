    ...
    if avaliable_check(user, form):
        ...
        if req.POST:           
            for q in qns:
                if q.multi_vars:
                    if req.POST.getlist(str(q.id)):
                        ...
                        txt = 'Изменения сохранены. <br><a href="/lk/forms/">Вернуться к списку форм</a>'
                    else:
                        if q.not_blank:
                            fl = False
                            wq.append(q)
            ...
            if fl:
                ...
                txt = 'Вы не ответили на обязательные вопросы (обязательные вопросы отмечены *)'
        if txt == '':
            txt = 'Заполните форму и нажмите "Сохранить".<br>Обязательные вопросы отмечены *.'	
        ...
        if user_identify(data[i][0], data[i][1], data[i][2], data[i][3]) != 'ok':
            ...
            if u.year == 0 or u.year == 1900 or u.year == 2100:
                ...
                for j in range(7, len(data[0])):
                    if data[0][j][:3].lower() == 'pdf':
                        if form.id in all_io and data[i][j] == 'yes':
                            diplom(u.user.id, data[0][j], '2018')
                            u.diploms = u.diploms + '</br>2018: <a href="/lk/forms/download/pdf/{}_2018">{}</a>'.format(data[0][j][4:data[0][j].find(' ')],
                ...
                if len(cdo) in range(1, 3):    # certificate disk (less then 3 subjects)
                    disk(u.user.id, 'по ' + ', '.join(cdo), '2018')
                    u.diploms = u.diploms + '</br>2018: <a href="/lk/forms/download/pdf/certificate_disk_2018">DisC</a>'
                elif len(cdo) > 2:
                    certificate(u.user.id,  'по ' + ', '.join(cdo), '2018')
                    u.diploms = u.diploms + '</br>2018: <a href="/lk/forms/download/pdf/certificate_zash_2018">Zash</a>'
                u.save()
