...
from lkregistration.users import UserModel, UserModelString
...
class RegistrationProfile(models.Model):
    ...
    def send_activation_email(self, site, request=None):
        ...
        subject = getattr(settings, 'REGISTRATION_EMAIL_SUBJECT_PREFIX', '') + \
                  render_to_string('lkregistration/activation_email_subject.txt', ctx_dict)
        # Email subject *must not* contain newlines
        subject = ''.join(subject.splitlines())

        message_txt = render_to_string('lkregistration/activation_email.txt', ctx_dict)
        email_message = EmailMultiAlternatives(subject, message_txt, settings.DEFAULT_FROM_EMAIL, [self.user.email])

        try:
            message_html = render_to_string('lkregistration/activation_email.html', ctx_dict)
